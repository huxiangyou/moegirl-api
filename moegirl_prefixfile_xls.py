...
params={'action':'query','format':'json','list':'prefixsearch','pslimit':500,'psnamespace':'6','pssearch':''}
...
pagetitlelist:list=[]
for sheet in sheets:
	pagetitlelist+=[c.value for c in list(sheet.columns)[2]]
...
print(__doc__.format(est=len(prefixlist)//10+1))
...
for prefix in prefixlist:
	...
	for pageJson in json['query']['prefixsearch']:
		...
		if title not in pagetitlelist:
			print("[新]","\t[Find]",prefix,"\t[FID]",pageid,"\t[Title]",title)
			pagetitlelist.append(title)
			n_new+=1
		else:
			#print("-","\t[FID]",pageid,"\t[Title]",title)
			pass
